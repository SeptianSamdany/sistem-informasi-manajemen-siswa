...
$result = mysqli_query($connection, "SELECT * FROM pembayaran_tahun_a2");
?>

<section class="section">
  <div class="section-header d-flex justify-content-between">
    <h1>Data Pembayaran Tahunan Kelas A2</h1>
            ...
            <table class="table table-hover table-striped w-100" id="table-1">
              ...
              <tbody>
                ...
                while ($data = mysqli_fetch_array($result)) :
                    $tagihan = is_numeric($data['tagihan']) ? (float)$data['tagihan'] : 0;
                    $angsuran_ke_1 = is_numeric($data['angsuran_ke_1']) ? (float)$data['angsuran_ke_1'] : 0;
                    $angsuran_ke_2 = is_numeric($data['angsuran_ke_2']) ? (float)$data['angsuran_ke_2'] : 0;
                    $angsuran_ke_3 = is_numeric($data['angsuran_ke_3']) ? (float)$data['angsuran_ke_3'] : 0;
                    $angsuran_ke_4 = is_numeric($data['angsuran_ke_4']) ? (float)$data['angsuran_ke_4'] : 0;
                    $angsuran_ke_5 = is_numeric($data['angsuran_ke_5']) ? (float)$data['angsuran_ke_5'] : 0;
                    $sisa_tagihan = is_numeric($data['sisa_tagihan']) ? (float)$data['sisa_tagihan'] : 0;
                ?>
                  <tr class="text-center">
                    <td><?= $data['nis'] ?></td>
                    <td><?= $data['nama_siswa'] ?></td>
                    <td>Rp <?= number_format($tagihan, 0, ',', '.') ?></td>
                    <td>Rp <?= number_format($angsuran_ke_1, 0, ',', '.') ?></td>
                    <td>Rp <?= number_format($angsuran_ke_2, 0, ',', '.') ?></td>
                    <td>Rp <?= number_format($angsuran_ke_3, 0, ',', '.') ?></td>
                    <td>Rp <?= number_format($angsuran_ke_4, 0, ',', '.') ?></td>
                    <td>Rp <?= number_format($angsuran_ke_5, 0, ',', '.') ?></td>
      ...
      buttons: [
        {
          ...
          pageSize: 'A4',
          title: 'Data Pembayaran Tahunan Kelas A2',
          exportOptions: {
